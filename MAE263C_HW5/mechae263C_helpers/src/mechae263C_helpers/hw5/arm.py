...
class CoriolisCentripetal(LeafSystem):
    def __init__(self, k3: float):
        ...
        self.DeclareVectorInputPort(name="qdot", size=2)
        self.DeclareVectorOutputPort(
            name="C(q, qdot) @ qdot", size=2, calc=self.calc_output
        )
...
class ResolvedAcceleration(LeafSystem):
    def __init__(
        ...
        self.load_mass_kg = load_mass_kg
    ...
    def calc_inertia_mat(self, cos_q1: float):
        ...
        if self.load_mass_kg is not None:
            inertia_mat[0, 1] += self.load_mass_kg * self.link_lens[0] ** 2
            inertia_mat[1, 0] += self.load_mass_kg * self.link_lens[1] ** 2
            inertia_mat[1, 1] += self.load_mass_kg * self.link_lens[1] ** 2
...
class Arm(Diagram):
    def __init__(self, load_mass_kg: float | None = None):
        super().__init__()
        ...
        L = 1
        self.links = [
            Link(length=L, mass=m, joint_axis_to_com_dist=L / 2, com_inertia=10),
            Link(length=L, mass=m, joint_axis_to_com_dist=L / 2, com_inertia=10),
        ]
        self.motors = [
            Motor(mass=5.0, com_inertia=0.01, transmission_ratio=float(K_r[0, 0])),
            Motor(mass=5.0, com_inertia=0.01, transmission_ratio=float(K_r[1, 1])),
        ]
    ...
    def get_q_output_port(self) -> OutputPort:
        """Returns the output port for joint positions"""
        return self.get_output_port(1)

    def get_qdot_output_port(self) -> OutputPort:
        """Returns the output port for joint velocities"""
