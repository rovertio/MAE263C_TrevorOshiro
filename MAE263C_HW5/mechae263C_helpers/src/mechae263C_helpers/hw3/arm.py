...
class CoriolisCentrifugal(LeafSystem):
...
class Arm(Diagram):
    def __init__(self, F_v: NDArray[np.double]):
        super().__init__()

        self.F_v = np.asarray(F_v, dtype=np.double)

        self.links = [
            Link(length=1, mass=50, joint_axis_to_com_dist=0.5, com_inertia=10),
            Link(length=1, mass=50, joint_axis_to_com_dist=0.5, com_inertia=10),
        ]
        self.motors = [
            Motor(mass=5, com_inertia=0.01, transmission_ratio=100),
            Motor(mass=5, com_inertia=0.01, transmission_ratio=100),
        ]
        ...
        self.augmented_links = [
            AugmentedLink(
                ...
                link_frame_inertia=I1 + m1 * (l1 - a1) ** 2 + self.motors[0].com_inertia,
            ),
            AugmentedLink(
                mass=m2,
        ...
        coriolis_centrifugal = diagram_builder.AddNamedSystem(
            ...
            CoriolisCentrifugal(k3=-self.links[0].length * self.dyn_params[2]),
        )
